# AGENTS.md — NamiOS Project Context
...
NamiOS is a **self-hosted AI assistant** — each user installs their own instance.
Every decision should favor simplicity and ease of self-hosting.
No shared auth or multi-tenancy needed.

## Project Overview

**NamiOS** (formerly Meow) is an always-on AI personal assistant running on Hetzner (4GB Ubuntu).
...
Tamagotchi-inspired personality system that gives each user's Nami a unique character.

    data/soul/SOUL.md          # Per-user personality file

- **First-run onboarding**: On first launch (no SOUL.md exists), Nami triggers an onboarding flow to learn the user's name, preferences, and communication style, then generates SOUL.md
